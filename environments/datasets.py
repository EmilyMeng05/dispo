...
class D4RLDataset(OfflineDataset):
    ...
    def __getitem__(self, idx):
        ...
        action_indices = np.arange(idx, idx + self.num_actions)  # Sequentially select the next actions
        ...
        action_sequences = actions[:self.num_actions]  # Use num_actions instead of hardcoded 4
        next_state = next_states[self.num_actions]   # Use num_actions to get the correct next state
        ...
        return dict(
            ...
            rewards=self.full_dataset["rewards"][idx + self.num_actions],  # Corresponding reward for the next state
            dones=self.full_dataset["dones"][idx + self.num_actions]     # Corresponding done flag for the next state
        )
